    ...
    <Box style={{backgroundColor: '#f3f4f6'}}>
      ...
      <Grid container spacing={2} sx={{ px: 3, mt: 2 }}>
        {getShown('top_products')?.map((p) => (
          <Grid item xs={12} sm={6} md={4} lg={3} key={`top_products-${p.product_id || p.product_name}`}>
            <NavLink to={`/productpage/${p.product_id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ px: 3 }}>
            {getShown('computers_accessories').map((p) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={`computers_accessories-${p.product_id || p.product_name}`}>
                <NavLink to={`/productpage/${p.product_id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <ProductCard product={p} />
                </NavLink>
              </Grid>
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ px: 3 }}>
            {getShown('electronics_products').map((p) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={`electronics_products-${p.product_id || p.product_name}`}>
                <NavLink to={`/productpage/${p.product_id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <ProductCard product={p} />
                </NavLink>
              </Grid>
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ px: 3 }}>
            {getShown('home_kitchen').map((p) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={`home_kitchen-${p.product_id || p.product_name}`}>
                <NavLink to={`/productpage/${p.product_id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <ProductCard product={p} />
                </NavLink>
              </Grid>
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ px: 3, mb: 6 }}>
            {getShown('office_products').map((p) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={`office_products-${p.product_id || p.product_name}`}>
                <NavLink to={`/productpage/${p.product_id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <ProductCard product={p} />
                </NavLink>
              </Grid>
          ...
          {productCardData.office_products.length > 12 ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, mb: 6 }}>
              <Button variant="outlined" onClick={() => toggle('office_products', productCardData.office_products.length)} style={{marginBottom: "48px"}}>
